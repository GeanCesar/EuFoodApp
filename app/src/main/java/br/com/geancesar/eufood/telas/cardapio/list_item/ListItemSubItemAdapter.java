...
public class ListItemSubItemAdapter extends RecyclerView.Adapter<ListItemSubItemAdapter.ViewHolder>{
    ...
    public void onBindViewHolder(@NonNull ListItemSubItemAdapter.ViewHolder holder, int position) {
        ...
        holder.csCheck.setSelecionado(itens.get(position).getQuantidadeSelecionada() > 0);

        // Mostra check
        if(categoria.getQuantidadeMinima() == 1 && categoria.getQuantidadeMaxima() == 1) {
            holder.csCheck.setVisibility(VISIBLE);
            holder.nsQuantidade.setVisibility(GONE);

            holder.csCheck.setListener(selecionado -> {
                if(selecionado) {
                    itens.get(position).setQuantidadeSelecionada(1);
                    limpaChecks(position);
                    listener.adicionouSubItem(itens.get(position));
                } else {
                    itens.get(position).setQuantidadeSelecionada(0);
                    listener.removeuSubItem(itens.get(position));
                }
            });

        } else { // Mostra number selector
            holder.csCheck.setVisibility(GONE);
            holder.nsQuantidade.setVisibility(VISIBLE);
        ...
        holder.tvPrecoItemCardapio.setVisibility(itens.get(position).getValor().compareTo(BigDecimal.ZERO) > 0 ? VISIBLE : GONE );
    ...
    private void limpaChecks(int position) {
        for(int i = 0; i < itens.size(); i++) {
            if(i != position) {
                itens.get(i).setQuantidadeSelecionada(0);
                notifyItemChanged(i);
            }
        }
    }
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        ViewHolder(View itemView) {
            ...
            csCheck = itemView.findViewById(R.id.csCheck);

            csCheck.setSelecionado(false);
        }
    }
